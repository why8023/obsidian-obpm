...
- Related document workflow: Run a command to move documents associated with project files or files inside project folders into a configured project subfolder when they are outside the project folder, with an undo command for the last move batch.
- Pinned relation target: Pin one Markdown file and automatically link newly created related files to it through the relation property.
