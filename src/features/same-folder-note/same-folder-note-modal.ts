...
interface SameFolderNotePromptOptions {
	cancelLabel: string;
	description: string;
	initialValue: string;
...
export class SameFolderNotePromptModal extends Modal {
	private errorEl: HTMLDivElement | null = null;
	private resolver: ((value: string | null) => void) | null = null;
	private resolved = false;
	private textComponent: TextComponent | null = null;

	constructor(app: App, private readonly options: SameFolderNotePromptOptions) {
		super(app);
	}
	...
	onOpen(): void {
		...
		const inputWrapperEl = this.contentEl.createDiv({cls: 'obpm-same-folder-note-modal-input'});
		...
		this.errorEl = this.contentEl.createDiv({
			cls: 'obpm-same-folder-note-modal-error setting-item-description',
		});
